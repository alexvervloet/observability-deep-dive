...
> **Everything in this repo runs offline.** No key, no network, no cost, and the
...
so the same "3σ is weird" rule works for latency, cost, and refusals alike, with no
...
**with a confidence interval**, since a dip inside the error bars isn't a regression yet.
...
> **There is no setting that gives zero false alarms *and* zero misses.** Every
...
Learn the vocabulary, since you'll be asked it, but don't buy a vendor's "LLM
...
`--healthy` it fires nothing. That gap, catching real incidents while ignoring noise, is
...
storage sane; and remember your logs are a **PII sink** (Production §3), so scrub
